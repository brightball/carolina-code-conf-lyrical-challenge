...
var she = new Woman { Expression = Dumb.Level["kinda"], ForeheadAdornment = HandGesture.LShape };   // C#

float years = mod(u_time, 120.);                                                                    // GLSL
rules |> Seq.iter (fun r -> hit(ground, Running))                                                   // F#
?- liveFor(notFun), false.                                                                          %! Prolog

def adjust(brain, head, i), do: {%{brain | smart: brain.smart + i}, %{head | dumb: head.dumb - i}}  # Elixir
